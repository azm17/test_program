...
def sql_data_send(login_id,login_pass,port,host,db_name,weight_after,weight_before,contents,time,moisture,tenki,shitsudo):
    data_list=[]
    conn = mysql.connector.connect(
        host = host,
        port = port,
...
def sql_data_get(login_id,login_pass,port,host,db_name):
    conn = mysql.connector.connect(
        host = host,
        port = port,
        user = login_id,
    ...
    if (not connected):
        conn.ping(True)
    cur.execute('''SELECT * FROM `{}` '''.format(login_id))
